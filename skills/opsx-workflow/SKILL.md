...
整个流程遵循 OpenSpec 的 change 目录结构：

```
openspec/changes/<change-name>/
├── proposal.md      ← 阶段1：需求澄清
├── design.md        ← 阶段2：架构设计（OpenSpec 原有）
├── tasks.md         ← 阶段2：任务规划
└── qa_report.md     ← 阶段4：质量门禁
```
...
1. **先思考再编码**：不确定就问，多种方案就列出来，有更简单的做法就直说
2. **简单优先**：只写解决问题的最少代码，不做推测性功能，不做过早抽象
3. **外科手术式变更**：只改必须改的，匹配现有风格，不重构无关代码
4. **目标驱动执行**：每个任务定义成功标准，循环验证直到通过
...
### 阶段 1：需求澄清（OpenSpec 驱动）
- **产出**：`openspec/changes/<name>/proposal.md`
- **活动**：通过 OpenSpec 创建 change → 澄清需求 → 编写 proposal → 用户确认后标记 `STATUS: APPROVED`
- **原则**：先思考再编码 — 不确定就问，不假设需求
- **约束**：禁止编码，禁止编辑源码

### 阶段 2：架构与规划
- **产出**：`design.md`（架构设计）+ `tasks.md`（任务列表）
- **活动**：
  1. 架构设计（写入 design.md）：目录结构、分层架构、核心领域模型、技术选型、接口定义
  2. Superpowers brainstorming：用苏格拉底式提问澄清设计中的模糊点
  3. 任务规划（写入 tasks.md）：拆分为可独立 TDD 的任务，每个任务包含明确的验收标准
- **原则**：简单优先 — 不做过早抽象，先有可运行的架构再迭代
- **约束**：禁止编码，禁止编辑源码。用户确认后标记 `STATUS: IN_PROGRESS`

### 阶段 3：编码执行（TDD 严格模式）
- **产出**：源码 + 测试代码
- **活动**：逐任务严格执行 TDD 三步循环：
  1. **Red**：写失败测试，明确期望行为（先思考再编码）
  2. **Green**：写最小实现让测试通过（简单优先）
  3. **Refactor**：消除重复、改善命名、保持简洁（外科手术式变更）
- **原则**：简单优先、外科手术式变更、目标驱动
- **约束**：全部任务完成 → `STATUS: DONE`，进入质量门禁

### 阶段 4：质量门禁（GStack 驱动）
- **产出**：`qa_report.md`
- **活动**：使用 GStack Skill 执行质量检查：
  - `/qa` — 功能测试、缺陷挖掘
  - `/review` — 代码规范性审查
  - `/cso` — 安全合规审计
  - `/health` — 代码健康度检查
  - `/investigate` — 问题定位分析
- **原则**：目标驱动 — 通过全部检查 → `RESULT: PASS`，否则 `RESULT: FAIL` 返回阶段 3
- **约束**：QA FAIL 禁止提交，必须返回阶段 3 修复后重新检查

### 阶段 5：提交归档（GStack 驱动）
- **产出**：git commit + 文档更新 + 归档
- **活动**：使用 GStack Skill 完成收尾：
  - `/ship` — 代码提交、分支处理、PR 创建
  - `/document-release` — 项目文档更新与发布说明
...
- **活动**：
  1. **架构设计**（写入 design.md）：目录结构、分层架构、核心领域模型、技术选型、接口定义
     - 新项目参考 `.claude/standards/ddd.md`（DDD 约束）
     - 现有项目参考 `.claude/standards/architecture.md`（已提取的架构约束）
  2. **Superpowers brainstorming**：用苏格拉底式提问澄清设计中的模糊点
  3. **任务规划**（写入 tasks.md）：拆分为可独立 TDD 的任务，每个任务包含明确的验收标准
- **原则**：简单优先 — 不做过早抽象，先有可运行的架构再迭代
- **约束**：禁止编码，禁止编辑源码。用户确认后标记 `STATUS: IN_PROGRESS`
...
- **活动**：逐任务严格执行 TDD 三步循环：
  ...
  3. **Refactor**：消除重复、改善命名、保持简洁（外科手术式变更）
- **原则**：
  - 简单优先：200 行能改成 50 行就改
  - 外科手术式变更：只改与任务相关的代码，不重构无关部分
  - 目标驱动：每个任务必须通过全部测试才算完成
- **约束**：全部任务完成 → `STATUS: DONE`，进入质量门禁
...
6. 所有产出落地文件，禁止口头交付
7. 需求变更立即终止返回阶段1
8. 阶段1/2 禁用 Superpowers，阶段3 启用
9. 阶段3 强制 TDD：Red(写测试) → Green(最小实现) → Refactor(重构)
10. 工具缺失或环境异常立即终止
